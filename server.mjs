...
async function handleStatus(response) {
  ...
  json(response, 200, {
    ...
    message: status.configured ? undefined : '尚未設定 OPENAI_API_KEY',
  });
}
...
async function handleDm(request, response) {
  try {
    ...
    const output = await runDungeonMaster(prompt, AbortSignal.timeout(90_000));
...
server.listen(port, '127.0.0.1', () => {
  ...
  const status = getAgentStatus();
  console.log(`OpenAI Agent: ${status.configured ? status.model : 'OPENAI_API_KEY not configured'}`);
});
