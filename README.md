...
需求：Go 1.24+、Node.js 20+，以及 Codex CLI（或 Grok CLI；都沒有可用示範 DM）。完整安裝模組盤點（含選用的本地圖片與語音）見 `REQUIREMENTS.md`；所有腳本說明見 `scripts/README.md`。
...
- SRD 5.2.1 的 6 個職業（吟遊詩人、牧師、戰士、聖武士、盜賊、法師），每個列出的特性都有真實伺服器機制（吟遊激勵、引導神力、聖擊、偷襲、秘法回復⋯）
...
- 劇本模式：四個故事模板內建分支模組（各 56–62 節點、5–6 種結局），選項按鈕推進、零 AI 延遲；自由模式維持 AI DM
- 公平骰運：必要檢定 DC 由伺服器夾在每位擲骰者 30–90% 成功率區間
- 一鍵把整場冒險匯出成第一人稱小說 `.txt`
- 選用 GPT-SoVITS 本地語音旁白（`/api/tts`，見 `scripts/sovits-setup.sh`）
...
2. 選擇故事模板（或自訂標題、場景、目標）；四個內建模板會再問「劇本模式或自由模式」——劇本模式照分支模組走、用選項按鈕行動，自由模式由 AI DM 即興。接著建立 1–4 名角色：6 個職業、種族、背景、能力值都可調整
...
- 開團選內建模板時可改走**劇本模式**：伺服器按預寫的分支模組推進，行動改成選項按鈕，回覆即時（不呼叫 AI）
- 每個模組有城鎮樞紐、一次性寶物與戰鬥、依隊伍人數縮放的敵人；每個選擇記一次傾向值
- 結局有 5–6 種（善／惡／中立），善終需要累積傾向值達門檻
- 每進入新節點全隊 +25 XP；幕完成時跳出成功視窗

### 小說匯出與語音

- 戰役紀錄頁可把整場冒險匯出成**第一人稱小說 `.txt`**
- 裝好 GPT-SoVITS（`scripts/sovits-setup.sh` + `scripts/sovits.sh`）後，DM 敘事可用本地語音旁白播放
...
後端測試涵蓋 DM 提示組建（與原 Node 版位元組相同）、DM 輸出驗證與強制結構、模型正規化、JS 語意轉換、劇本模組編譯與推進、SQLite 圖片存取（含並發寫入）與 HTTP 端點；測試不會呼叫 Codex。前端測試涵蓋 6 職業角色建立、法術與法術位、資源和休息、動態隊伍回合與開團表單。`.github/workflows/ci.yml` 在 push／PR 時跑同一套測試（後端含 `-race`，前端含 build）。
