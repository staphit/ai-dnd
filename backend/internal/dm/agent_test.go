...
func TestRunDungeonMasterReturnsValidatedTurn(t *testing.T) {
	...
	turn, err := RunDungeonMaster(context.Background(), api, "input", "", "", "schema.json", ".", "story1", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestRunDungeonMasterRequiresLogin(t *testing.T) {
	...
	_, err := RunDungeonMaster(context.Background(), api, "input", "", "", "schema.json", ".", "story1", false)
