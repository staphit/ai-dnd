              ...
              <div key={slot.id} className={`scene-gallery-item${selected ? ' selected' : ''}${slot.imageUrl ? '' : ' scene-slot-pending'}`}>
                {slot.imageUrl ? (
                  ...
                  >
                    ...
                    <span className="scene-gallery-label">{rendering ? '生成中…' : `生成・${label}`}</span>
                  </button>
                )}
              </div>
            );
